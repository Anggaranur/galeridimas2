...
<html lang="en">
	...
	<body>
		...
		<div onclick="history.back()" style="cursor: pointer; margin: 40px 0 0 40px; width: fit-content; padding: 18px; border-radius: 99px; color: white; background: #222; align-items: center; font-size: 18px; display: flex; gap: 12px;">
       ...
       $sql=mysqli_query($koneksi, "select * from likefoto where fotoid='$fotoid' and userid='$userid'");
							...
							<a href="hapusfoto.php?fotoid=<?php echo $data['fotoid']?>&albumid=<?php echo $data['albumid']?>" onclick="return confirm('Apakah anda yakin ingin hapus foto?')">
